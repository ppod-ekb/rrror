...
@AllArgsConstructor
@Component
public class DbServiceZuulFilterAction {
                ...
                action
                        ...
                        .getMessage((ctx) ->
                                Optional.of(Message.create(getDestination.apply(Pair.with(createPrefix.get(), ctx)),
                                        new HalParser(
                                                new ContextData(ctx).get()).parse().getSelfLink()))
                        )
                ...
                action
                        ...
                        .modify((ctx) ->
                                new ContextData(ctx).write(
                                        new HalHrefModifier(
                                                new ContextData(ctx).get(), config.getDbServiceConfig().getUrlPrefix()).modify()
                                )
                        );
                return action;
            });
    ...
    public List<Runnable> getActions() {
        ...
        /*
        RequestCtxPredicate predicate = new RequestCtxPredicate() {};
        HrefModifierFunction hrefModifierFunction = new HrefModifierFunction() {};
        EntityExtractorFunction entityExtractorFunction = new EntityExtractorFunction() {};
        ...
        NotifyAction createNewEntityNotifyAction = new NotifyAction();
        NotifyAction updateEntityNotifyAction = new NotifyAction();
        NotifyAction deleteEntityNotifyAction = new NotifyAction();
        ...
        LinkModifyAction linkModifyAction = new LinkModifyAction();
        linkModifyAction
                .on(status200.and(getMethod.or(putMethod).or(postMethod))
                        .or(status201.and(putMethod.or(postMethod))))
                .modify((ctx) ->
                        new ContextData(ctx).write(
                                new HalHrefModifier(
                                        new ContextData(ctx).get(), config.getDbServiceConfig().getUrlPrefix(), hrefModifierFunction).modify()
                        )
                );
    ...
    static class ContextData implements ContextDataReader,ContextDataWriter {
